...
The maze is represented by a 2D array. The player is represented by 'X'. The path that the player can move is on '.'. "T" represents the treasury box, you will get bonus points but lose steps when you encounter it. "M" represents the monster, you will get bonus points and more steps when you encounter it. If you encounter '#', that is a wall, which cannot be passed through. You must first obtain the key 'K'. Get to the exit 'E' to escape the maze.
